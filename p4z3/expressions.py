...
def resolve_expr(p4_state, expr_chain, expr) -> z3.SortRef:
    ...
    if isinstance(val, P4Z3Class):
        ...
        return step(p4_state, [val] + expr_chain)
...
def get_type(p4_state, expr_chain, expr):
    ''' Return the type of an expression, Resolve, if needed'''
    arg_expr = resolve_expr(p4_state, expr_chain, expr)
...
def z3_implies(p4_state, expr_chain, cond, then_expr):
    no_match = step(p4_state, expr_chain)
...
def align_bitvecs(bitvec1, bitvec2, p4_state, expr_chain):
    if isinstance(bitvec1, z3.BitVecRef) and isinstance(bitvec2, z3.BitVecRef):
        if bitvec1.size() < bitvec2.size():
            bitvec1 = P4Cast(bitvec1, bitvec2).eval(p4_state, expr_chain)
        if bitvec1.size() > bitvec2.size():
            bitvec2 = P4Cast(bitvec2, bitvec1).eval(p4_state, expr_chain)
...
class MethodCallExpr(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        ...
        if isinstance(p4_method, P4Action):
            ...
            return step(p4_state, [p4_method] + expr_chain)
...
class P4BinaryOp(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        ...
        rval_expr = resolve_expr(p4_state, expr_chain, self.rval)
        ...
        lval_expr, rval_expr = align_bitvecs(
            lval_expr, rval_expr, p4_state, expr_chain)
...
class P4Mux(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        condition = resolve_expr(p4_state, expr_chain, self.condition)
        then_val = resolve_expr(p4_state, expr_chain, self.then_val)
        else_val = resolve_expr(p4_state, expr_chain, self.else_val)
    ...
    def eval(self, p4_state, expr_chain):
        ...
        if isinstance(self.members, dict):
            for name, val in self.members.items():
                val_expr = resolve_expr(p4_state, expr_chain, val)
            ...
            for index, rval in enumerate(self.members):
                ...
                val_expr = resolve_expr(p4_state, expr_chain, rval)
...
class SliceAssignment(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        ...
        return step(p4_state, [assign] + expr_chain)
    ...
    def eval(self, p4_state, expr_chain):
        ...
        if isinstance(self.rval, list):
            ...
            for val in self.rval:
                rval_expr = resolve_expr(p4_state, expr_chain, val)
                # any assignment copies the variable
                # do not pass references
                rval_expr = deepcopy(rval_expr)
                list_expr.append(resolve_expr(p4_state, expr_chain, val))
            p4_state.set_list(self.lval, list_expr)
        else:
            rval_expr = resolve_expr(p4_state, expr_chain, self.rval)
            # any assignment copies the variable
            # do not pass references
            ...
            if isinstance(rval_expr, int):
                lval = p4_state.resolve_reference(self.lval)
...
class MethodCallStmt(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        return step(p4_state, [self.method_expr] + expr_chain)
...
class BlockStatement(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        return step(p4_state, self.exprs + expr_chain)
...
class P4Action(P4Z3Class):
    ...
    def merge_args(self, p4_state, expr_chain, *args):
    ...
    def eval(self, p4_state, expr_chain):
        ...
        return step(p4_state, expr_chain, expr)
    ...
    def eval(self, p4_state, expr_chain):
        ...
        for index, arg in enumerate(self.args):
            ...
            if is_ref:
                ...
                arg_type = get_type(p4_state, expr_chain, arg)
...
class P4Control(P4Action):
    ...
    def eval(self, p4_state=None, expr_chain=[]):
        ...
        # execute the action expression with the new environment
        expr = step(p4_state_tmp, self.block)
        for param in self.params:
            is_ref = param[0]
            param_name = param[1]
...
class IfStatement(P4Z3Class):
    ...
    def eval(self, p4_state, expr_chain):
        ...
        cond = resolve_expr(p4_state, expr_chain, self.cond)
        ...
        then_expr = step(then_p4_state, [self.then_block] + expr_chain)
        if self.else_block:
            else_p4_state = deepcopy(p4_state)
            else_expr = step(else_p4_state, [self.else_block] + expr_chain)
            return z3.If(cond, then_expr, else_expr)
        else:
            return z3_implies(p4_state, expr_chain, cond, then_expr)
...
class SwitchHit(P4Z3Class):
    ...
    def eval_loop(self, p4_state, expr_chain, cases):
        ...
        if cases:
            ...
            return step(p4_state_copy, [self.default_case] + expr_chain)
...
class SwitchStatement(P4Z3Class):
    ...
    def eval_loop(self, p4_state, expr_chain, cases):
        ...
        if cases:
            _, case = cases.popitem(last=False)
            case_expr = step(p4_state_copy, [case["case_block"]] + expr_chain)
            then_expr = self.eval_loop(p4_state, expr_chain, cases)
            return z3.If(case["match"], case_expr, then_expr)
        else:
            return step(p4_state_copy, [self.default_case] + expr_chain)
    ...
    def eval(self, p4_state, expr_chain):
        ...
        return step(p4_state, [table, switch_hit] + expr_chain)
...
class P4Table(P4Z3Class):
    ...
    def apply(self, p4_state, expr_chain):
        return step(p4_state, [self] + expr_chain)
    ...
    def eval_action(self, p4_state, expr_chain, action_tuple):
        ...
        p4_action.merge_args(p4_state, expr_chain, p4_action_args)
        return step(p4_state, [p4_action] + expr_chain)

    def eval_default(self, p4_state, expr_chain):
        ...
        return self.eval_action(p4_state, expr_chain,
                                (action_name, p4_action_args))

    def eval_const_loop(self, p4_state, expr_chain, const_entries):
        if not const_entries:
            return self.eval_default(p4_state, expr_chain)
        ...
        for index, key in enumerate(self.keys):
            key_eval = resolve_expr(p4_state, expr_chain, key)
            c_key_eval = resolve_expr(p4_state, expr_chain, const_keys[index])
            matches.append(key_eval == c_key_eval)
        action_match = z3.And(*matches)
        action_expr = self.eval_action(p4_state, expr_chain,
                                       (p4_action, p4_action_args))
        then_expr = self.eval_const_loop(
            p4_state, expr_chain, const_entries)
        return z3.If(action_match, action_expr, then_expr)

    def eval_table_loop(self, p4_state, expr_chain, actions):
        if not actions:
            const_entries = self.const_entries.copy()
            return self.eval_const_loop(p4_state, expr_chain, const_entries)
        ...
        then_expr = self.eval_table_loop(p4_state, expr_chain, actions)
        return z3.If(action_match, action_expr, then_expr)
    ...
    def eval(self, p4_state, expr_chain):
        ...
        tbl_match = self.eval_keys(p4_state, expr_chain)
        actions = self.actions.copy()
        table_expr = self.eval_table_loop(p4_state, expr_chain, actions)
        def_expr = self.eval_default(p4_state, expr_chain)
