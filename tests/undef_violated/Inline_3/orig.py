...
def p4_program(z3_reg):
    ...
    z3_reg.declare_global(
        ControlDeclaration(P4Control(
            ...
            type_params=[],
            params=[
                P4Parameter("out", "reset", z3.BitVecSort(48), None),],
            const_params=[],
            body=BlockStatement([]
            ),
            local_decls=[]
    ...
    z3_reg.declare_global(
        ControlDeclaration(P4Control(
            ...
            const_params=[],
            body=BlockStatement([
                MethodCallStmt(MethodCallExpr(P4Member("reset_0", "apply"), [], P4Member(P4Member("hdr", "eth_hdr"), "src_addr"), )),
                IfStatement(P4eq(P4Member(P4Member("hdr", "eth_hdr"), "src_addr"), z3.BitVecVal(1, 48)), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "eth_hdr"), "eth_type"), z3.BitVecVal(1, 16)),]
                ), ),
                IfStatement(P4eq(P4Member(P4Member("hdr", "eth_hdr"), "eth_type"), z3.BitVecVal(1, 16)), BlockStatement([
                    AssignmentStatement(P4Member(P4Member("hdr", "eth_hdr"), "dst_addr"), z3.BitVecVal(1, 48)),]
                ), ),]
            ),
            local_decls=[
InstanceDeclaration("reset_0", "reset_ctrl", ), ]
        ))
    )
    z3_reg.declare_global(
        ControlDeclaration(P4Control(
