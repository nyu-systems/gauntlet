...
control ingress(inout Headers h, inout Meta m, inout standard_metadata_t sm) {
    ...
    @name("ingress.do_action") action do_action() {
        ...
        }
        {
            ...
            {
                {
                    h.eth_hdr.eth_type = (cond_0 ? val_0 : h.eth_hdr.eth_type);
                    tmp = (cond_0 ? (pointless_bool_0 ? 48w1 : 48w2) : tmp);
                }
            }
            {
                val_0 = (cond_0 ? h.eth_hdr.eth_type : val_0);
            }
            h.eth_hdr.src_addr = (cond_0 ? tmp : h.eth_hdr.src_addr);
        }
    }
    apply {
        do_action();
